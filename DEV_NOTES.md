...
- solver.py        ; MDP value-iteration optimizer. Caches per-state action sets
                      (self._action_cache) so value iteration doesn't recompute the
                      eligible-mod distribution every pass; ~13x speedup, identical
...
- trade_client.py  ; live pathofexile.com trade2 client, currency-normalized to Exalted.
...
- no DEPLOY_MODE (python app.py / .exe): market features on with user's POESESSID.
  Badge: "DESKTOP · MARKET LIVE".
- /api/config tells the UI which mode it's in.
